...
export const saveCardapio = async (cardapio: Cardapio): Promise<void> => {
  try {
    ...
    const cardapioRef = doc(db, "apps", "comanda-direta", "lojistas", cardapio.id, "cardapios", "principal");
...
export const getCardapio = async (lojaId: string): Promise<Cardapio | null> => {
  try {
    ...
    const cardapioRef = doc(db, "apps", "comanda-direta", "lojistas", lojaId, "cardapios", "principal");

    // Buscando em paralelo
    const [lojistaSnap, cardapioSnap] = await Promise.all([
        getDoc(lojistaRef),
        getDoc(cardapioRef)
    ]);
  ...
  const produtosTeste: Produto[] = [
    {
      ...
      descricao: "Pão brioche, 2 hambúrgueres de 150g, bacon crocante, queijo cheddar e maionese da casa.",
      preco: 29.90,
      categoria: "Lanches",
      imagemUrl: "https://imgs.search.brave.com/3rSVij0jhsZYblY1eMa7x23QcjmtaqUjZJ4DL9yClJA/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9pbWFn/ZXMucGV4ZWxzLmNv/bS9waG90b3MvMTM4/NzAzNy9wZXhlbHMt/cGhvdG8tMTM4NzAz/Ny5qcGVnP2F1dG89/Y29tcHJlc3MmY3M9/dGlueXNyZ2ImZHBy/PTEmdw01MDA",
    },
    {
      id: "prod_2",
      nome: "Coca-Cola Lata",
